	...
	for file_name in file_v
		...
		while !CSV.eof(file.io)
			...
			if occursin("Ref", rf[1]) && occursin("Outcome", rf[2]) && session != :not_interesting
				...
				elseif session == :train && session_to_analyse == :train
					if first_time
						println(file.name)
						first_time = false ;
					end

					push!(dt_v, get_train_subj(subject_dt, subject_id, file_name, path)) ;
				end
			end
		end
		if !isempty(dt_v)
...
function klimb_mi(path::String, session_to_analyse::Symbol, n_trials_in_the_past::Int64)
	...
	switch_after_incorr_v = Array{Float64,1}() ;
	n_switch_v = Array{Int64,1}() ;
	n_switch_binom_v = Array{Int64,1}() ;
	...
	for file_name in file_v
		...
		while !CSV.eof(file.io)
			...
			if occursin("AC Comment", rf[1])
				if occursin("Probe", rf[3]) && occursin("midpoint", rf[3])
					session = :probe ;
					dt_row_len = 20 ;
				elseif occursin("Probe", rf[3]) && occursin("1 vs 1", rf[3])
					session = :probe_1vs1 ;
			...
			if occursin("Ref", rf[1]) && occursin("Outcome", rf[2]) && session != :not_interesting
				...
				if session == :probe && session_to_analyse == :probe
					...
					tp = prev_trial_pairing_amb_tone(subject_dt, subject_id, i, 0)
					#tp = amb_trial_pairing(subject_dt, subject_id, 0)
					append!(curr_press_v, tp[:,1]) ;
					append!(past_press_v, tp[:,2]) ;
					append!(past_reward_v, tp[:,3]) ;
					push!(n_trials_v, length(tp[:,1])) ;

					switch_dt = get_switch_after_incorr(tp[:,1], tp[:,2], tp[:,3], tp[:,4]) ;

					push!(switch_after_incorr_v, switch_dt[1]) ;
					push!(n_switch_v, switch_dt[2]) ;
					push!(n_switch_binom_v, switch_dt[3]) ;
		
				elseif	session == :probe_1vs1 && session_to_analyse == :probe

					tp = prev_trial_pairing_amb_tone(subject_dt, subject_id, i, 2)
					#tp = amb_trial_pairing(subject_dt, subject_id, 2)
					append!(curr_press_v, tp[:,1]) ;
					append!(past_press_v, tp[:,2]) ;
					append!(past_reward_v, tp[:,3]) ;
					push!(n_trials_v, length(tp[:,1])) ;
				...
				elseif session == :train && session_to_analyse == :train
					...
					tp = prev_trial_pairing_base_tone(subject_dt, subject_id, i, 0)
					if length(tp[:,1]) != 0
						append!(curr_press_v, tp[:,1]) ;
						append!(past_press_v, tp[:,2]) ;
						append!(past_reward_v, tp[:,3]) ;
	...
	figure()
	hist(switch_after_incorr_v, 25)
	xlabel("% switches", fontsize = 14)
	ylabel("N [session]", fontsize = 14)
	show()
	
	
	n_significant_switch = 0 ;
	for j = 1 : length(switch_after_incorr_v)
		if n_switch_v[j] > n_switch_binom_v[j]
			n_significant_switch += 1 ;
		end
	end
	println(n_significant_switch, " ", length(switch_after_incorr_v))
...
function get_train_subj(subject_dt::Array{Int64,2}, subject_id::Int64, session::String, in_file::String, in_path::String)
	...
	if mod(subject_id,2) == 0
		...
		subject_vec[3:end] = [mean(corr_II_RT)/100.0, mean(corr_I_RT)/100.0, mean(incorr_II_RT)/100.0, mean(incorr_I_RT)/100.0,
							...
							100.0*miss_II/(n_II + miss_II), 100.0*miss_I/(n_I + miss_I), 
							100.0*prem/(subject_dt[end,1]+1)] ;
		...
		subject_vec[3:end] = [mean(corr_I_RT)/100.0, mean(corr_II_RT)/100.0, mean(incorr_I_RT)/100.0, mean(incorr_II_RT)/100.0,
						...
						100.0*miss_I/(n_I + miss_I), 100.0*miss_II/(n_II + miss_II), 
						100.0*prem/(subject_dt[end,1]+1)] ;
...
function get_probe_subj(subject_dt::Array{Int64,2}, subject_id::Int64, in_file::String, in_path::String, col_offset::Int64)
	...
	if mod(subject_id,2) == 0
		...
		subject_vec[2:end] = [mean(corr_II_RT)/100.0, mean(corr_I_RT)/100.0, mean(incorr_II_RT)/100.0, mean(incorr_I_RT)/100.0,
							...
							100.0*miss_II/(n_II + miss_II), 100.0*miss_I/(n_I + miss_I), 
							100.0*miss_amb/(n_amb + miss_amb), 100.0*prem/(subject_dt[end,1]+1), mean([amb_I_RT ; amb_II_RT])/100.0] ;
...
function amb_trial_pairing(subject_dt::Array{Int64,2}, subject_id::Int64, col_offset::Int64)
	...
	for i = 2 : size(subject_dt,1)
		...
		if subject_dt[i,2] == 0
			if subject_dt[i,17 + col_offset] != 0 	# cue 1 press
				mod(subject_id,2) == 0 ? curr_press = 1 : curr_press = 4
				consider_trial = true ;
			elseif subject_dt[i,19 + col_offset] != 0	# cue 2 press
				mod(subject_id,2) == 0 ? curr_press = 4 : curr_press = 1
				consider_trial = true ;
			end

		elseif subject_dt[i,2] == 1
			if subject_dt[i,17 + col_offset] != 0	# cue 2 press
				mod(subject_id,2) == 0 ? curr_press = 4 : curr_press = 1
				consider_trial = true ;
			end

		elseif subject_dt[i,2] == 3
			if subject_dt[i,19 + col_offset] != 0	# cue 1 press
				mod(subject_id,2) == 0 ? curr_press = 1 : curr_press = 4
				consider_trial = true ;
			end
		end
		...
		if consider_trial
			while past_trial >= 1 && !found_past_trial
				if subject_dt[past_trial,2] == 0
					if subject_dt[past_trial,17 + col_offset] != 0 # cue 1 press correctly
						if mod(subject_id,2) == 0 
							past_press = 1 ;
							past_reward = 1 ;
						else
					...
					elseif subject_dt[past_trial,19 + col_offset] != 0 # cue 2 press correctly
						if mod(subject_id,2) == 0 
							past_press = 4 ;
							past_reward = 4 ;
						else
				...
				elseif subject_dt[past_trial,2] == 1 && subject_dt[past_trial,17 + col_offset] != 0 # cue 2 press incorrectly
					if mod(subject_id,2) == 0 
						past_press = 4 ;
						past_reward = 0 ;
					else
					...
					found_past_trial = true ;
				elseif subject_dt[past_trial,2] == 3 && subject_dt[past_trial,19 + col_offset] != 0 # cue 1 press incorrectly
					if mod(subject_id,2) == 0 
						past_press = 1 ;
						past_reward = 0 ;
					else
				...
				elseif subject_dt[past_trial,2] == 2 && 
					(subject_dt[past_trial,17 + col_offset] != 0 || subject_dt[past_trial,19 + col_offset] != 0)	# omission
					past_press = 0 ;
					past_reward = 0 ;
					found_past_trial = true ;
				end
...
function prev_trial_pairing_amb_tone(subject_dt::Array{Int64,2}, subject_id::Int64, n_trials_in_the_past::Int64, 
							col_offset::Int64)
	...
	for i = n_trials_in_the_past + 1 : size(subject_dt,1)
		...
		if subject_dt[i,2] == 0
			if subject_dt[i,17 + col_offset] != 0 	# cue 1 press
				mod(subject_id,2) == 0 ? curr_press = 1 : curr_press = 4
				RT = subject_dt[i,18 + col_offset] - subject_dt[i,17 + col_offset] ;

				consider_trial = true ;
			elseif subject_dt[i,19 + col_offset] != 0	# cue 2 press
				mod(subject_id,2) == 0 ? curr_press = 4 : curr_press = 1
				RT = subject_dt[i,20 + col_offset] - subject_dt[i,19 + col_offset] ;
		...
		if consider_trial
			if subject_dt[past_trial,2] == 0
				if subject_dt[past_trial,13 + col_offset] != 0 || subject_dt[past_trial,17 + col_offset] != 0 # cue 1 press correctly
					if mod(subject_id,2) == 0 
						past_press = 1 ;
						past_reward = 1 ;
					else
				...
				elseif subject_dt[past_trial,15 + col_offset] != 0 || subject_dt[past_trial,19 + col_offset] != 0 # cue 2 press correctly
					if mod(subject_id,2) == 0 
						past_press = 4 ;
						past_reward = 4 ;
					else
	...
	return Any[curr_press_v past_press_v past_reward_v RT_v]
end

function prev_trial_pairing_base_tone(subject_dt::Array{Int64,2}, subject_id::Int64, n_trials_in_the_past::Int64, 
							col_offset::Int64)
	...
	for i = n_trials_in_the_past + 1 : size(subject_dt,1)
		...
		if subject_dt[i,2] == 0
			if subject_dt[i,15 + col_offset] != 0 	# cue 1 press
				mod(subject_id,2) == 0 ? curr_press = 1 : curr_press = 4
				consider_trial = true ;
			elseif subject_dt[i,17 + col_offset] != 0	# cue 2 press
				mod(subject_id,2) == 0 ? curr_press = 4 : curr_press = 1
				consider_trial = true ;
			end

		elseif subject_dt[i,2] == 1
			if subject_dt[i,15 + col_offset] != 0	# cue 2 press
				mod(subject_id,2) == 0 ? curr_press = 4 : curr_press = 1
				consider_trial = true ;
			end
		...
		if consider_trial
			if subject_dt[past_trial,2] == 0
				if subject_dt[past_trial,15 + col_offset] != 0 # cue 1 press correctly
					if mod(subject_id,2) == 0 
						past_press = 1 ;
						past_reward = 1 ;
					else
				...
				elseif subject_dt[past_trial,17 + col_offset] != 0 # cue 2 press correctly
					if mod(subject_id,2) == 0 
						past_press = 4 ;
						past_reward = 4 ;
					else
...
function get_switch_after_incorr(curr_press_v::Array{Any,1}, past_press_v::Array{Any,1}, 
								past_reward_v::Array{Any,1}, RT_v::Array{Any,1})

	n_incorr_before_amb = 0 ;
	n_switch = 0 ;
	RT_switch_v = Array{Float64,1}() ;
	RT_not_switch_v = Array{Float64,1}() ;

	for i = 1 : length(curr_press_v)
		if past_reward_v[i] == 0
			n_incorr_before_amb += 1 ;
			if curr_press_v[i] != past_press_v[i]
				n_switch += 1 ;
				push!(RT_switch_v, RT_v[i])
			else
				push!(RT_not_switch_v, RT_v[i])
			end
		end
	end
	...
	while cum_prob < 0.05
		cum_prob += binomial(n_incorr_before_amb, n_switch_binom) * p^n_switch_binom * 
							(1.0 - p)^(n_incorr_before_amb - n_switch_binom) ;
		n_switch_binom -= 1 ;
	end


	if n_switch > n_switch_binom && length(RT_switch_v) > 1 && length(RT_not_switch_v) > 1
		#println((mean(RT_switch_v) - mean(RT_not_switch_v)) / sqrt(std(RT_switch_v)^2.0 + std(RT_not_switch_v)^2.0))
	end

	return [100.0*n_switch/n_incorr_before_amb n_switch n_switch_binom]
end